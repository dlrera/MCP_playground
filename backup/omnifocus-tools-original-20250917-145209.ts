    ...
    if (args.project) {
      ...
      script += `
        ...
        if targetTask is missing value then
          try
            set targetTask to first task whose name is "${args.taskName}"
          end try
        end if`;
    }
  ...
  async getTaskNote(args: GetTaskNoteArgs) {
    ...
    if (args.project) {
      script += `
        ...
        if targetProject is not missing value then
          set targetTask to first task of targetProject whose name is "${args.taskName}" and completed is false
        else
          return "ERROR: Project not found: ${args.project}"
        end if`;
    } else {
      script += `
        ...
        repeat with proj in every project
          try
            set matchingTask to first task of proj whose name is "${args.taskName}" and completed is false
        ...
        if targetTask is missing value then
          try
            set targetTask to first inbox task whose name is "${args.taskName}" and completed is false
          end try
        end if`;
    }
